...
async function generateDocumentation(config: CrawlConfig) {
  ...
  const pagesSelector = "aside .toc li a";
  await page.waitForSelector(pagesSelector, { timeout: 0 });
  const pageUrls = await page.$$eval(pagesSelector, (pagesLinks) =>
    pagesLinks.map((link) => (link as HTMLAnchorElement).href)
  );
